...
import br.ufpr.inf.gres.sentinel.strategy.operation.impl.select.GroupSelectionOperation;
    ...
    public Operation createOperation(Option node, Iterator<Integer> cyclicIterator) {
        ...
        switch (rule.getName()) {
            ...
            case TerminalRuleType.SELECT_OPERATORS_BY_GROUPS:
                Preconditions.checkArgument(rules.hasNext(), "Malformed grammar option: " + node.toString());
                rule = rules.next();
                GroupSelectionOperation<Operator> groupOperation = (GroupSelectionOperation<Operator>) FactoryFlyweight.getNonTerminalFactory().createOperation(rule, cyclicIterator);
