 ...
 * @version 1.0
 */
public class HypervolumeCalculator<S extends Solution<?>> extends Hypervolume<S> {
    ...
    public List<S> computeHypervolumeContribution(List<S> solutionList, List<S> referenceFrontList) {
        return new PISAHypervolume().computeHypervolumeContribution(solutionList, referenceFrontList);
    }

    @Override
    public Double evaluate(List<S> evaluate) {
        return calculateHypervolume(evaluate);
    }
}
